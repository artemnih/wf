...
export function updateStepsWithScripts(
	...
	jobs: Job[],
	fileHandler = (script: object, runId: string, basePath: string = cwlFsPath) => {
		// MONGO does not allow $ as key names.
		// We need to rename namespaces to $namespaces and schemas to $schemas.
		writeFileSync(`${basePath}/${runId}.cwl`, JSON.stringify(replaceKeyName(script as Record<string, unknown>)));
