...
export class SlurmCli implements HpcCli {
	...
	statusOfJob(id: string): JobHPC {
		const jobStatus = spawnSync('sacct', ['-n', '-p', '--delimiter=,', '--format=Start,End,State', '--name', id]);
		console.log(`Running sacct -n -p --delimiter=, --format=Start,End,State --name ${id}`);
		return jobFromHpc(id, jobStatus.output.toString(), getInputsFromCwl, getOutputsFromCwl);
	}
