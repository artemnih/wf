...
describe('OutputsFromCwl', () => {
    ...
    expect(
      getOutputsFromCwl(
        ...
        (file: string) => {
          return JSON.parse(
            readFileSync(
              'src/__tests__/data/doubles/6171ac4137b0b158411138e4.out.json',
              'utf8',
            ),
          ) as OutputDict;
        },
  ...
  it('zarr-format', () => {
    ...
    const parameterHandler = (file: string) => {
      return JSON.parse(
        readFileSync(
          'src/__tests__/data/doubles/scatter-format.out.json',
          'utf8',
        ),
      ) as OutputDict;
    };
  ...
  it('Output While Running', () => {
    const workflowHandler = (file: string) => {
      return JSON.parse(
        readFileSync(
          'src/__tests__/data/doubles/dynamic-scatter-compute.cwl',
          'utf8',
        ),
      ) as CwlWorkflowTemplate;
    };
  ...
  it('Dynamic Scatter', () => {
    ...
    expect(
      ...
      filePatterns: [
        'x24_y16_wx2_wy1_c{c}.ome.tif',
        'x24_y16_wx2_wy2_c{c}.ome.tif',
      ],
    });
  });
});
