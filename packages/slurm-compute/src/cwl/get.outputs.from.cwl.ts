  ...
  if (foundValue) {
    ...
    for (const [, value] of Object.entries(stepEntry.out)) {
      if (!Array.isArray(outputs[value])) {
        ...
        returnObject[value] = locationFromOutput(
          arrayValue as unknown as OutputDict,
        );
      ...
      if (
        !returnObject[value] &&
        (value.includes('StdOut') || value.includes('StdErr'))
      ) {
        require('dotenv').config();
        const slurmConfig = require('config');
        const jobNameLog =
          scatterIndex === 0 ? jobName : jobName + '_' + (scatterIndex + 1);
