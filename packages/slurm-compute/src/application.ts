...
import {SlurmComputeApiBindings} from './keys';
export class SlurmComputeApplication extends BootMixin(
  ...
  setUpBindings(options: ApplicationConfig): void {
    this.bind('datasources.config.ApiDS').to(options.argoCompute?.db);
    this.bind(SlurmComputeApiBindings.CONFIG).to(options);
