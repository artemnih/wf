...
describe('AddOperatorPlugin', () => {
	it('Add Operator no scatter', () => {
		...
		const cwlScriptInAndOut = scriptsFromWorkflow(helloWorld1Workflow, helloWorldJobs);
		expect(addOperatorPlugin(helloWorld1Workflow, cwlScriptInAndOut, helloWorldJobs)).to.be.eql({
	...
	it('dynamic scatter', () => {
		...
		const operatorReturn = addOperatorPlugin(dynamicScatter, scripts, initialJobs);
