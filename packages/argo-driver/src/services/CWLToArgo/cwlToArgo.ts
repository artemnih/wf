 ...
 */
export function cwlToArgo(cwlWorkflow: CwlWorkflow, cwlJobInputs: CwlJobInputs, computeJobs: ComputeJob[]): ArgoWorklow {
	...
	const generatedTemplates = steps.map(step => {
	...
	const argoWorkflow = buildArgoWorkflowTemplate(cwlWorkflow, containers, tasks);
